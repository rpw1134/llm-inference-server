...
export function SettingsModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
      <div className="relative w-full max-w-5xl rounded-3xl border border-slate-800 bg-neutral-950 p-6 shadow-2xl shadow-black/60">
        <button
          onClick={onClose}
        ...
        <div className="mb-5 flex items-start justify-between gap-4">
          ...
          <div className="flex items-center gap-3 rounded-2xl border border-slate-800 bg-neutral-900/70 px-4 py-3 text-xs text-slate-300">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <div className="md:col-span-2 space-y-3 rounded-2xl border border-slate-800 bg-neutral-900/60 p-4">
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="space-y-2 text-sm font-medium text-slate-200">
                <span className="block text-slate-300">Repo ID</span>
                <input
                  className="w-full rounded-xl border border-slate-800 bg-neutral-950 p-3 text-sm text-white outline-none focus:border-orange-400 focus:ring-2 focus:ring-orange-400/20"
                  value={value.repo_id}
                  onChange={(e) =>
          ...
          <div className="rounded-2xl border border-slate-800 bg-neutral-900/60 p-4 text-sm text-slate-200">
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {numberInputs.map((input) => (
            <label
              key={input.key}
              className="space-y-2 rounded-xl border border-slate-800 bg-neutral-900/80 p-3"
            >
              ...
              <input
                ...
                value={value.model_params[input.key]}
                onChange={(e) => handleParamChange(input.key, e.target.value)}
                className="w-full rounded-lg border border-slate-800 bg-neutral-950 p-2 text-sm text-white outline-none focus:border-orange-400 focus:ring-2 focus:ring-orange-400/20"
              />
            </label>
          ))}
        </div>
